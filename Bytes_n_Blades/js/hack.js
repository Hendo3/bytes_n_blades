...
const HackSystem = {
    ...
    createUI() {
        ...
        overlay.innerHTML = `
            <div class="hack-terminal" role="dialog" aria-modal="true" aria-labelledby="hack-title" aria-describedby="hack-msg">
                <h2 id="hack-title">>> ${tr("hack.title", "BREACH PROTOCOL")} <<</h2>
                <div class="hack-status" id="hack-msg" role="status" aria-live="polite">${tr("hack.injecting", "INJECTING MALWARE... STOP IN THE BLUE ZONE")}</div>
        ...
        const action = document.getElementById("btn-hack-stop");
        action.onclick = () => this.stop();
        if (typeof window !== "undefined" && window.CyberDialogs) {
            window.CyberDialogs.activate(overlay, {
                dialog: overlay.querySelector(".hack-terminal"),
                initialFocus: action,
                returnFocus: this.returnFocusTarget,
                onEscape: () => this.close(this.pendingResult ?? false),
            });
        } else {
            action.focus();
            overlay.addEventListener("keydown", (event) => {
                ...
                this.close(this.pendingResult ?? false);
            });
        }
    },
    ...
    stop() {
        ...
        if (hit) {
            ...
            this.closeTimer = setTimeout(() => this.close(true), 1000);
        } else {
            msg.textContent = typeof window !== "undefined" && window.I18n
            ...
            this.closeTimer = setTimeout(() => this.close(false), 1000);
        }
    },
    ...
    close(success) {
        ...
        if (typeof window !== "undefined" && window.CyberDialogs) {
            window.CyberDialogs.deactivate(overlay);
        } else if (this.returnFocusTarget?.isConnected) {
