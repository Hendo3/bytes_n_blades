...
(function (root) {
  ...
  function comparisonUrl(page, id) {
    if (root.CyberPermalinks) return root.CyberPermalinks.catalogItemUrl(id, {}, new URL(page, root.location?.href).href);
    const base = root.location?.href || `https://bytes.invalid/html/${page}`;
  ...
  function sourceLabel(source) {
    ...
    return [source.book || source.title, source.pages ? `p. ${source.pages}` : null].filter(Boolean).join(" · ") || null;
  }

  function createProgramComparison(program, classLabel) {
    return {
      catalog: "programs",
      id: program.id,
      name: program.name,
      category: classLabel || program.class_id || t("compare.family.programs", "PROGRAMS"),
      description: program.effect,
      price: program.price,
      url: comparisonUrl("programs.html", program.id),
      stats: [
        { key: "strength", label: t("net.strength", "STR"), value: strengthLabel(program) },
        { key: "memory", label: "MU", value: memoryLabel(program) },
        { key: "platform", label: t("compare.platform", "PLATFORM"), value: program.platform },
        { key: "availability", label: t("compare.availability", "AVAILABILITY"), value: program.availability },
        { key: "capacity", label: t("compare.capacity", "CAPACITY"), value: program.capacity },
        { key: "source", label: t("compare.source", "SOURCE"), value: sourceLabel(program.source) },
      ],
    };
  }

  function createDeckComparison(deck) {
    return {
      catalog: "cyberdecks",
      id: deck.id,
      name: deck.name,
      category: t("compare.family.cyberdecks", "CYBERDECKS"),
      description: deck.description,
      price: deck.price,
      url: comparisonUrl("cyberdecks.html", deck.id),
      stats: [
        { key: "cpu", label: "CPU", value: deck.cpu },
        { key: "memory", label: "MU", value: deck.memory },
        { key: "speed", label: t("net.speed", "Speed"), value: deck.speed },
        { key: "data_wall", label: t("net.data_wall", "Data Wall"), value: deck.data_wall },
        { key: "armor", label: "SP", value: deck.armor_sp },
        { key: "portable", label: t("compare.portable", "PORTABLE"), value: deck.portable ? t("compare.yes", "YES") : t("compare.no", "NO") },
        { key: "cellular", label: t("compare.cellular", "CELLULAR"), value: deck.cellular ? t("compare.yes", "YES") : t("compare.no", "NO") },
        { key: "options", label: t("compare.options", "OPTIONS"), value: (deck.options || []).join(" · ") },
        { key: "availability", label: t("compare.availability", "AVAILABILITY"), value: deck.availability },
        { key: "source", label: t("compare.source", "SOURCE"), value: sourceLabel(deck.source) },
      ],
    };
  }

  function appendCompareButton(card, candidate) {
    ...
    button.addEventListener("click", () => {
      if (root.CyberComparator) root.CyberComparator.toggle(candidate);
      else root.dispatchEvent?.(new root.CustomEvent("comparator-request", { detail: candidate }));
    });
    card.appendChild(button);
    root.CyberComparator?.syncButton(button);
    return button;
  }

  function appendShareButton(card, itemId, page) {
    if (!root.CyberPermalinks) return null;
    const button = root.CyberPermalinks.createButton(
      () => root.CyberPermalinks.catalogItemUrl(itemId, {}, new URL(page, root.location?.href).href),
  ...
  function renderProgramCard(program, classes) {
    ...
    card.dataset.itemId = program.id;
    card.id = `item-${program.id}`;
    ...
    card.innerHTML = `
      ...
      <p class="editorial-source">${t("compare.source", "SOURCE")} // ${sourceLabel(program.source)}</p>
    ...
    card.appendChild(button);
    appendShareButton(card, program.id, "programs.html");
    appendCompareButton(card, createProgramComparison(program, classLabel));
    return card;
  }
  ...
  function initPrograms(data) {
    ...
    const linkedFilters = root.CyberPermalinks?.readProgramState() || {};
    if (search && linkedFilters.query) search.value = linkedFilters.query;
    if (maxPrice && linkedFilters.maxPrice) maxPrice.value = linkedFilters.maxPrice;
    if (conversions) conversions.checked = Boolean(linkedFilters.includeConversions);
    const requestedItem = new URLSearchParams(root.location.search).get("item");
    if (requestedItem && data.programs.some((program) => program.id === requestedItem && program.catalog_scope === "netrunner_conversion")) {
      conversions.checked = true;
    }
    ...
    if (classSelect && [...classSelect.options].some((option) => option.value === linkedFilters.classId)) {
      classSelect.value = linkedFilters.classId;
    }
    ...
    const render = () => {
      const visible = filterPrograms(data.programs, currentFilters());
      ...
      focusRequestedItem(list);
    };
    [classSelect, search, maxPrice, conversions].forEach((field) => field?.addEventListener("input", () => {
      render();
      if (root.CyberPermalinks) root.CyberPermalinks.replace(root.CyberPermalinks.programFilterUrl(currentFilters()));
    }));
    if (root.CyberPermalinks && !document.querySelector("[data-share-program-filter]")) {
      const share = root.CyberPermalinks.createButton(
        () => root.CyberPermalinks.programFilterUrl(currentFilters()),
        t("permalink.filter", "COPY FILTER LINK"),
      );
      share.dataset.shareProgramFilter = "true";
      document.querySelector(".net-filters")?.appendChild(share);
    }
    render();
  }
  ...
  function initDecks(data) {
    ...
    const renderDeck = (deck) => {
      ...
      card.dataset.itemId = deck.id;
      card.id = `item-${deck.id}`;
      card.innerHTML = `
        ...
        <p class="editorial-source deck-source">${t("compare.source", "SOURCE")} // ${sourceLabel(deck.source)}</p>
      ...
      card.appendChild(button);
      appendShareButton(card, deck.id, "cyberdecks.html");
      appendCompareButton(card, createDeckComparison(deck));
      return card;
    };
    const linkedDeckFilters = root.CyberPermalinks?.readProgramState() || {};
    if (conversionToggle) conversionToggle.checked = Boolean(linkedDeckFilters.includeConversions);
    const requestedItem = new URLSearchParams(root.location.search).get("item");
    if (requestedItem && data.decks.some((deck) => deck.id === requestedItem && deck.catalog_scope === "netrunner_conversion")) {
    ...
    conversionToggle?.addEventListener("input", () => {
      render();
      if (root.CyberPermalinks) {
        root.CyberPermalinks.replace(root.CyberPermalinks.programFilterUrl({
          includeConversions: conversionToggle.checked,
        }));
      }
    });
    if (root.CyberPermalinks && conversionToggle) {
      conversionToggle.closest("label")?.after(root.CyberPermalinks.createButton(
        () => root.CyberPermalinks.programFilterUrl({ includeConversions: conversionToggle.checked }),
        t("permalink.filter", "COPY FILTER LINK"),
      ));
    }
    render();
  ...
  function focusRequestedItem(container) {
    const requestedItem = new URLSearchParams(root.location.search).get("item");
    if (!requestedItem || !container) return;
    const target = Array.from(container.querySelectorAll("[data-item-id]"))
  ...
  function initBuilder(programData, deckData) {
    ...
    const linkedBuild = root.CyberPermalinks?.readBuilderState() || null;

    if (linkedBuild) {
      if ([...chassis.options].some((option) => option.value === linkedBuild.chassisId)) chassis.value = linkedBuild.chassisId;
      if ([...connection.options].some((option) => option.value === linkedBuild.connectionId)) connection.value = linkedBuild.connectionId;
      if ([...speed.options].some((option) => option.value === linkedBuild.speed)) speed.value = linkedBuild.speed;
      if ([...wall.options].some((option) => option.value === linkedBuild.dataWall)) wall.value = linkedBuild.dataWall;
      form.elements.usedStandard.checked = linkedBuild.usedStandard;
      form.elements.expandedMemory.checked = linkedBuild.expandedMemory;
      form.querySelectorAll("[data-option]").forEach((field) => {
        const value = linkedBuild.options[field.dataset.option];
        if (value === undefined) return;
        if (field.type === "checkbox") field.checked = value === true || value === "true" || value === "1";
        else if ([...field.options || []].some((option) => option.value === String(value)) || field.type === "number") field.value = String(value);
      });
    }
    ...
    if (conversionToggle && linkedBuild) {
      const optionalPrograms = new Set(programData.programs
        .filter((program) => program.catalog_scope === "netrunner_conversion")
        .map((program) => program.id));
      conversionToggle.checked = linkedBuild.includeConversions
        || linkedBuild.programIds.some((id) => optionalPrograms.has(id));
    }
    ...
    renderPrograms();
    if (linkedBuild) {
      const selectedPrograms = new Set(linkedBuild.programIds);
      form.querySelectorAll("[data-program]").forEach((field) => {
        field.checked = selectedPrograms.has(field.dataset.program);
      });
    }
    ...
    if (root.CyberPermalinks && addButton) {
      addButton.after(root.CyberPermalinks.createButton(
        () => root.CyberPermalinks.builderUrl(readBuilderSelection(form), conversionToggle?.checked),
        t("permalink.build", "COPY BUILD LINK"),
      ));
    }
    let current;
    const update = () => {
      ...
      if (linkedBuild && root.CyberPermalinks) {
        root.CyberPermalinks.replace(root.CyberPermalinks.builderUrl(
          readBuilderSelection(form),
          conversionToggle?.checked,
        ));
      }
    };
  ...
  const api = {
    ...
    createProgramComparison, createDeckComparison, readBuilderSelection, initPrograms, initDecks, initBuilder, loadData,
  };
  root.Netrunning = api;
